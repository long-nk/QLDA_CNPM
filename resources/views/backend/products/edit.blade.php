                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            <form class="form-horizontal form-label-left" action="{{route('products.update', $product->id)}}"
                                ...
                                <div class="item form-group">
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Loại sản phẩm
                                        <span class="required">*</span>
                                    </label>
                                    <div class="col-md-3 col-sm-6 col-xs-12">
                                        <select name="category" id="" class="form-control">
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}" {{$product->C_id == $category->id?"selected":""}}>{{$category->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <div class="box_show_img">
                                            <img src="{{@$product->fileItem->url}}" alt="" id="img_show">
                                            <i class="">+</i>
                                        </div>
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-3 col-sm-6 col-xs-12">
                                        <select name="status" id="status" value="{{$product->status}}"
                                                class="form-control">
                                            <option value="1" {{$product->status == 1?"selected":""}}>Hiển thị
                                            </option>
                                            <option value="0" {{$product->status == 0?"selected":""}}>Không hiển
                                                thị
                                            </option>
                                        </select>
                                    </div>
